<template>
  <div class="menu-card">
    <!-- IMAGE -->
    <div class="img-box">
      <img :src="data.image + '?w=400&auto=format&fit=crop'" loading="lazy" />
    </div>

    <!-- CONTENT -->
    <div class="content">
      <h5>{{ data.title }}</h5>
      <p class="sub_title">{{ data.subtitle }}</p>
      <div class="divider">
        <span>What We're Serving </span>
      </div>

      <!-- ITEMS -->
      <div v-for="(item, i) in data.items" :key="item.name" class="menu-item">
        <div>
...
.menu-item {
  ...
  align-items: start;
  padding: 8px 0;
  border-bottom: 1px dashed $theme_g;
